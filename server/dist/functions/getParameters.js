...
const configDB_1 = require("../routes/faps/configDB");
const population = [
    {
        id: 1,
...
const getParameters = (params) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let connection = mysql2_1.default.createPool(configDB_1.configDB);
        const result = [];
        console.log(params);
        ...
        if (params.hasOwnProperty('area') && params.area !== null) {
            const areaName = yield connection.promise().query('SELECT * FROM `district` WHERE `id` = ' + params.area)
                .then(([rows]) => {
                return rows[0].name;
            })
                .catch(console.log);
        ...
        if (params.hasOwnProperty('type_id') && params.type_id !== null) {
            const typeName = yield connection.promise().query('SELECT * FROM `types` WHERE `id` = ' + params.type_id)
                .then(([rows]) => {
                console.log(rows);
                return rows[0].name;
            })
